...
app.post('/getResult', (req, res) => {
    ...
    if ('keywords' in reqBody) {
        ...
        if ('filters' in reqBody) {
            ...
            // 有关键词，无筛选条件
            db_1.default.query(`select good_id,price,title,images from goodInfo where available=0 and title like '%${reqBody.keywords.split('').join('%')}%'`, (err, result) => {
                if (err)
                    throw err;
...
app.get(`/getRecommendList`, (req, res) => {
    db_1.default.query(`select good_id,price,title,images from goodInfo where available=0 order by rand() limit 8`, (err, result) => {
        if (err)
            throw err;
        res.end(JSON.stringify(result));
    });
