...
<template>
  <!--身份验证窗口-->
  <n-card v-show="!atModify" content-style="display: flex;flex-direction: column;align-items: center;padding-bottom: 0;"
    header-style="padding-bottom: 0;" footer-style="padding: 0 6em 1em 8em" class="retrieve-card">
    <template #header>
      <el-page-header title="返回登录" @back="$emit('close-retrieve')" />
    </template>
    <p class="retrieve-title">用户找回密码</p>
    <n-form ref="retrieveRef" :model="retrieveData" :rules="rules" label-placement="left" label-width="auto">
      <n-form-item label="账号" path="userID">
        <n-input v-model:value="retrieveData.userID" :placeholder="''" class="input" />
      </n-form-item>
      <n-form-item label="手机号" path="telNum">
        <n-input v-model:value="retrieveData.telNum" :placeholder="''" class="input" />
      </n-form-item>
      <n-form-item label="验证码" path="CAPTCHA">
        <n-input v-model:value="retrieveData.CAPTCHA" :placeholder="''" class="input" />
      </n-form-item>
    </n-form>
    <template #footer>
      <div class="btns-wrapper">
        <n-button type="info" @click="goModify">下一步</n-button>
        <n-button type="success" @click="getCAPTCHA">获取验证码</n-button>
      </div>
    </template>
  </n-card>
  ...
  <n-card v-show="atModify" content-style="display: flex;flex-direction: column;align-items: center;padding-bottom: 0;"
    ...
    <n-form ref="modifyRef" :model="modifyData" :rules="modifyRules" label-placement="left" label-width="auto">
      <n-form-item label="新密码" path="password">
        <n-input v-model:value="modifyData.password" :placeholder="''" class="input" />
      </n-form-item>
      <n-form-item label="手机号" path="telNum">
        <n-input v-model:value="retrieveData.telNum" :placeholder="''" class="input" />
      </n-form-item>
    ...
    <template #footer>
      <div class="btns-wrapper">
        <n-button type="success" @click="completeModify">确认修改</n-button>
      </div>
    </template>
  </n-card>
</template>
