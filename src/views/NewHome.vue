...
<template>
  ...
  <el-container class="search-wrapper">
    <n-input class="search-input" v-model.trim="keyWord" placeholder="Type to Search." @keyup.enter="searchKeyWord">
      <template #prefix>
        <n-icon>
          <Search />
        </n-icon>
      ...
      <template #suffix>
        <n-button secondary type="info" size="small" round @click="searchKeyWord">搜索</n-button>
      </template>
    </n-input>
  </el-container>
  ...
  <el-container class="menu-wrapper">
    <el-row>
      ...
      <el-menu class="el-menu-vertical category-menu">
        <el-menu-item v-for="(item, index) in categories" :index="index.toString()" @click="jumpMenu(item)">
          <el-icon :size="20">
      ...
      <!--大图走马灯-->
      <el-carousel height="280px" class="top-carousel">
      ...
      <!--校区菜单-->
      <el-menu class="el-menu-vertical campus-menu">
        <el-menu-item v-for="(item, index) in campusList" :index="index.toString()" @click="jumpMenu(item)">
          <el-icon :size="20">
  ...
  <el-container class="mid-wrapper">
    <el-row>
      ...
      <div class="winter-wrapper">
        ...
        <n-button text-color="#fff" color="#C0ECFB" ghost class="winter-btn">
          前往
        </n-button>
      </div>

      <!--商品走马灯-->
      <el-carousel v-if="goodsList.length > 0" type="card" indicator-position="none" height="240px"
        class="good-carousel">
        <el-carousel-item v-for="item in goodsList" :key="item.id" class="good-carousel-item">
          <el-card :body-style="{ padding: 0 }" class="good-card" @click="jumpCard(item.id)">
            <el-image :src="item.path" class="good-img" fit="scale-down" />
            <p class="good-title">
              {{ item.name }}
            </p>
            <p class="price">
      ...
      <div class="dash-board-wrapper">
        <el-avatar class="avatar" shape="circle" :src="avatarUrl" :size="50" @click="clickAvatar">
        </el-avatar>
        <p class="hello-text">
          {{ identity === 'visitor' ? `Hi, please log in!` : `Welcome, ${userID}!` }}
        </p>
        <div class="nums-wrapper">
          <div class="num-item" v-for="item in profileData" @click="item.onClick(item.value)">
            <p :class="['item-numeric', identity !== 'member' ? 'color-grey' : '']">{{ item.value }}</p>
            <p class="item-label">{{ item.label }}</p>
          </div>
        </div>
        <div class="browse-wrapper">
          <div class="browse-item" v-for="item in browseData">
            <el-icon class="point" :size="24" :color="iconColor(item.value)">
              <star v-if="item.value === 'star'" @click="$router.push('/favorite')" />
              <clock v-else @click="$router.push('/favorite')" />
            </el-icon>
            <p class="item-label">{{ item.label }}</p>
          </div>
        </div>
      ...
      <div class="panel-wrapper">
        <!--游客-->
        <div class="panel" v-if="identity === 'visitor'">
          <p class="panel-text" style="color: #99CCCC;">PLEASE</p>
          <n-button class="panel-button" text-color="#fff" ghost color="#C0ECFB" @click="$router.push('/login')">
            登录
          </n-button>
        ...
        <!--普通用户-->
        <div v-else-if="identity === 'member'" class="panel">
          <n-button class="panel-button" v-for="item in memberButtons" :key="item.label" text-color="#fff" ghost
            color="#C0ECFB" @click="item.action">
            <template #icon>
        ...
        <!--管理员-->
        <div v-else-if="identity === 'admin'" class="panel">
          <n-button class="panel-button" v-for="item in adminButtons" :key="item.label" text-color="#fff" ghost
            color="#C0ECFB" @click="item.action">
            <template #icon>
              <n-icon :size="18" color="#FFF">
