        ...
        <el-col :span="4">
          <div class="nav-select">
            <el-select v-model="category" placeholder="Select">
              <el-option v-for="(item, index) in categories" :key="index" :value="item">
              </el-option>
            </el-select>
          </div>
        </el-col>
    ...
    <div class="Card" v-for="order of orderListView" :key="order.orderId">
      ...
      <div class="CardBody">
        <el-row>
          ...
          <el-col :span="3">
            <span class="Consult point" @click="$emit('show-report-modal', order.orderId, order.reported)">
          ...
          <el-col :span="4">
            ...
            <span v-else-if="order.status === '待确认'" class="Option point" @click="confirmReceipt(order.orderId)">
              <el-icon :size="13" class="Icon">
                <CircleCheck />
              </el-icon>
              确认收货
