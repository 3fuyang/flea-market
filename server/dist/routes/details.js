...
app.get('/checkAvailable/:good_id', (req, res) => {
    db_1.default.query(`select available from goodInfo where good_id=?`, [req.params.good_id], (err, result) => {
        if (err)
            throw err;
        let message = true;
...
app.get('/getGoods/:good_id', (req, res) => {
    new Promise((resolve) => {
        db_1.default.query("select * from goodInfo where good_id =?", [req.params.good_id], (err, result) => {
            if (err)
                throw err;
        ...
        .then((baseInfo) => {
        db_1.default.query(`select count(*) as cnt from collectionBox where good_id=?`, [req.params.good_id], (err, result) => {
            if (err)
                throw err;
...
app.get(`/getTrends`, (req, res) => {
    db_1.default.query(`select good_id,price,title,images from goodInfo where available=0 order by rand() limit 4`, (err, result) => {
        if (err)
            throw err;
